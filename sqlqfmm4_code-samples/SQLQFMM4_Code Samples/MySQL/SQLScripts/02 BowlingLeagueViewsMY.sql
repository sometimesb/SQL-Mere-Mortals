-- This requires that the tables from 00 BowlingLeagueStructureMY.sql be created first.
...
FROM ztblBowlerRatings, (SELECT Bowlers.BowlerID, Bowlers.BowlerLastName, Bowlers.BowlerFirstName, CAST(AVG(Bowler_Scores.RawScore) AS Signed Integer) AS BowlerAverage 
FROM Bowlers
INNER JOIN Bowler_Scores 
ON Bowlers.BowlerID=Bowler_Scores.BowlerID 
